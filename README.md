...
- **지표 채택 이유**
  ...
  - 낮은 별점이지만 본문이 긍정적인 경우에는 별점 입력 실수, 사용자별 평가 기준 차이 등 여러 원인 가능성을 검토하기 위한 기초 데이터로 활용할 수 있습니다.
...
- **괴리 리뷰 전용 CLI 필터**
  - 향후 python main.py list --discrepancy-only 옵션을 추가하여 앞서 정의한 괴리 조건에 해당하는 리뷰만 별도로 조회할 수 있도록 개선할 수 있습니다.
  - 필요할 경우 해당 리뷰를 별도 CSV로 내보내는 기능으로 확장할 수 있습니다.
...
### 시계열 차트 날짜 전처리 및 일/주/월 집계 기준

#### 현재 실제 코드 동작 방식

- **날짜 표기법 표준화 (`src/cli.py`, `src/cleaner.py`)**
  - 원본 리뷰 데이터는 입력 파일에 따라 날짜 문자열 형식이 서로 다를 수 있습니다.
  - 전처리 단계에서는 지원 가능한 날짜 형식을 파싱한 뒤 `YYYY-MM-DD` 형식으로 정규화하여 데이터베이스의 일자 컬럼에 일관되게 저장합니다.
  - 이를 통해 날짜 정렬 및 시계열 분석에 사용할 수 있는 일관된 날짜 형식을 유지합니다.

- **차트 데이터 집계 기준 (`src/visualizer.py`)**
  - 일별 평균 별점을 단일 선으로 표현하는 방식이 아니라, `review_date`와 감정 분석 결과 라벨(`positive`, `neutral`, `negative`)을 기준으로 그룹화합니다.
  - 같은 날짜의 리뷰를 감정별로 묶어 각 감정의 일일 리뷰 발생 건수를 집계하고, 이를 다중 꺾은선 차트로 시각화합니다.
  - 이를 통해 특정 날짜에 어떤 감정의 리뷰가 증가하거나 감소했는지 일자별 반응 추이를 비교할 수 있습니다.

- **리뷰 미등록 일자 0건 보정 (`reindex(..., fill_value=0)`)**
  - 리뷰가 등록되지 않은 날짜를 생략하고 데이터가 존재하는 날짜끼리 연결하면 시간 흐름을 정확하게 파악하기 어려울 수 있습니다.
  - 현재 구현에서는 데이터의 시작일부터 종료일까지 연속된 날짜 범위를 생성한 뒤 `reindex(..., fill_value=0)`을 적용합니다.
  - 따라서 리뷰가 없는 날짜도 감정별 리뷰 수를 0건으로 채워 연속적인 시간 축을 유지합니다.

#### 향후 개선 방안

- **주간(Week) 및 월간(Month) 단위 집계 옵션**
  - 현재 시계열 차트는 일 단위로 집계합니다.
  - 일별 리뷰 수가 적은 경우 소수의 리뷰만으로 감정별 건수가 크게 변동하여 단기적인 노이즈가 발생할 수 있습니다.
  - 향후 주간 및 월간 단위로 데이터를 묶어 집계할 수 있는 옵션을 추가하여 단기 변동을 완화하고 중·장기적인 감정 변화 흐름을 확인할 수 있도록 개선할 수 있습니다.
